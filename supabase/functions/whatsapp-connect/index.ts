...
// Always-on bridge service (see /whatsapp-bridge) that holds the persistent
// WhatsApp Web session. Configure with:
//   npx supabase secrets set WHATSAPP_BRIDGE_URL=... WHATSAPP_BRIDGE_TOKEN=...
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const bridgeRes = await fetch(`${BRIDGE_URL}/sessions/${user.id}/connect`, {
      method: "POST",
      headers: { Authorization: `Bearer ${BRIDGE_TOKEN}` },
    });

    if (!bridgeRes.ok) {
      return Response.json({ error: "Failed to start WhatsApp session" }, { status: 502, headers: corsHeaders });
    }
